...
namespace sc
{
    public sealed class Logger
    {
        ...
        internal void LogChatMessage(bool echo, string message, ulong chatGroupID = 0, ulong chatID = 0,
            ulong steamID = 0, [CallerMemberName] string previousMethodName = null)
        {
            if (string.IsNullOrEmpty(message) || (chatGroupID == 0 || chatID == 0) && steamID == 0)
            {
                LogNullError(nameof(message) + " || " + "((" + nameof(chatGroupID) + " || " + nameof(chatID) + ") && " +
                             nameof(steamID) + ")");
            ...
            var loggedMessage =
                new StringBuilder(previousMethodName + "() " + message + " " + (echo ? "->" : "<-") + " ");

            if (chatGroupID != 0 && chatID != 0)
            {
                loggedMessage.Append(chatGroupID + "-" + chatID);

                if (steamID != 0) loggedMessage.Append("/" + steamID);
            }
            else if (steamID != 0)
            {
            ...
            var logEventInfo = new LogEventInfo(LogLevel.Trace, nameof(Debug), loggedMessage.ToString());
