...
namespace sc
{
    public class SCHandler : ClientMsgHandler
    {
        ...
        private void HandleItemAnnouncements(IPacketMsg packetMsg)
        {
            if (packetMsg == null)
            {
                sc.Logger.LogNullError(nameof(packetMsg));
        ...
        private void HandleRedeemGuestPassResponse(IPacketMsg packetMsg)
        {
            ...
            var response = new ClientMsgProtobuf<CMsgClientRedeemGuestPassResponse>(packetMsg);
        ...
        private void HandleSharedLibraryLockStatus(IPacketMsg packetMsg)
        {
            ...
            var response = new ClientMsgProtobuf<CMsgClientSharedLibraryLockStatus>(packetMsg);
        ...
        private void HandleVanityURLChangedNotification(IPacketMsg packetMsg)
        {
            ...
            var response = new ClientMsgProtobuf<CMsgClientVanityURLChangedNotification>(packetMsg);
        ...
        internal async Task<bool> IgnoreFriend(ulong steamID)
        {
            ...
            var request = new CPlayer_IgnoreFriend_Request {steamid = steamID};
        ...
        internal async Task<bool> AddFriend(ulong steamID)
        {
            ...
            var request = new CPlayer_AddFriend_Request {steamid = steamID};
            SteamUnifiedMessages.ServiceMethodResponse response;
        ...
        public void SetCurrentMode(uint chatMode)
        {
            ...
            var request = new ClientMsgProtobuf<CMsgClientUIMode>(EMsg.ClientCurrentUIMode)
            {
                Body =
                {
                    chat_mode = chatMode
        ...
        internal sealed class UserNotificationsCallback : CallbackMsg
        {
            ...
            {
                if (jobID == null || msg == null) throw new ArgumentNullException(nameof(jobID) + " || " + nameof(msg));
                ...
                if (msg.notifications == null) return;
            ...
            internal UserNotificationsCallback([NotNull] JobID jobID, [NotNull] CMsgClientItemAnnouncements msg)
            {
                ...
                {
                    {EUserNotification.Items, msg.count_new_items}
                };
            }
        ...
        private sealed class PlayingSessionStateCallback : CallbackMsg
        {
            private readonly bool PlayingBlocked;
        ...
        private sealed class RedeemGuestPassResponseCallback : CallbackMsg
        {
            private readonly EResult Result;

            internal RedeemGuestPassResponseCallback([NotNull] JobID jobID,
                [NotNull] CMsgClientRedeemGuestPassResponse msg)
            {
                if (jobID == null || msg == null) throw new ArgumentNullException(nameof(jobID) + " || " + nameof(msg));
        ...
        private sealed class SharedLibraryLockStatusCallback : CallbackMsg
        {
            private readonly ulong LibraryLockedBySteamID;

            internal SharedLibraryLockStatusCallback([NotNull] JobID jobID,
                [NotNull] CMsgClientSharedLibraryLockStatus msg)
            {
                if (jobID == null || msg == null) throw new ArgumentNullException(nameof(jobID) + " || " + nameof(msg));

                JobID = jobID;

                if (msg.own_library_locked_by == 0) return;

                LibraryLockedBySteamID =
                    new SteamID(msg.own_library_locked_by, EUniverse.Public, EAccountType.Individual);
            }
        }

        private sealed class VanityURLChangedCallback : CallbackMsg
        {
            private readonly string VanityURL;

            internal VanityURLChangedCallback([NotNull] JobID jobID,
                [NotNull] CMsgClientVanityURLChangedNotification msg)
            {
                if (jobID == null || msg == null) throw new ArgumentNullException(nameof(jobID) + " || " + nameof(msg));
